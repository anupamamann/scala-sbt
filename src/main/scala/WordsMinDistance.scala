...
object WordsMinDistance {
   ...
   *         >0 min Window Length
   */
  def apply(sc:SparkContext,filePath: String, searchWords: List[String]): Int = {
    ...
    val minDistance: Int =
      ...
      if (filePath == null) {
        ...
        val words = try {
          sc.textFile(filePath, sc.defaultParallelism).
            flatMap(c => c.mkString.split("""\W+""")).collect()
        } catch {
        ...
        val pairs = words.toList.zip(words.indices).groupBy {
          case (k, v) => k
        }.filter(k => searchWords.contains(k._1.toLowerCase))
          .map {
          case (word, indexList) =>
            (indexList.map(_._2))
        }.toList
  ...
  def main(args: Array[String]) {
    ...
    if (args.length < 2)
      ...
      val searchWords = args(1).toString.toLowerCase.split( """\W+""").toList
      println("Window length:" + WordsMinDistance(sc,args(0), searchWords))
